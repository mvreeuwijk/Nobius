...
If you are editing templates and still need high-fidelity re-import with `import_mobius.py`, also read [Round-Trip Invariants](RoundTripInvariants.md). A template can remain valid for Möbius while still breaking Nobius round-trip reconstruction if it removes the structural markers the importer expects.
