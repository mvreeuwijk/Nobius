...
def test_load_config_merges_nested_override_values(tmp_path):
    ...
    assert config["profiles"]["problem_set"]["render"]["theme_location"] == "/themes/custom"
    assert config["profiles"]["problem_set"]["render"]["scripts_location"] == DEFAULT_CONFIG["profiles"]["problem_set"]["render"]["scripts_location"]
...
def test_validate_render_config_rejects_placeholder_defaults():
    ...
    config["profiles"]["problem_set"]["render"]["theme_location"] = "__SET_ME_THEME_LOCATION__"
    config["profiles"]["problem_set"]["render"]["scripts_location"] = "__SET_ME_SCRIPTS_LOCATION__"

    with pytest.raises(ValueError, match="theme_location, scripts_location"):
        validate_render_config(config, "problem_set")


def test_generate_group_cli_fails_cleanly_when_render_config_is_placeholder(t01_sheet, tmp_path):
    config_path = tmp_path / "nobius.json"
    payload = make_config_payload(problem_set_theme_location="__SET_ME_THEME_LOCATION__")
...
def test_generate_group_cli_fails_cleanly_when_explicit_profile_is_placeholder(t01_sheet, tmp_path):
    config_path = tmp_path / "nobius.json"
    write_json(
        config_path,
        make_config_payload(exam_theme_location="__SET_ME_THEME_LOCATION__"),
    )
