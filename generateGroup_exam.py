...
##### GLOBALS #####
class consts:
    SCRIPTS_LOCATION = "web/Cive4000002/Public_Html/Scripts/QuestionJavaScript.txt"
...
def process_custom_response(identifier, part, q_title, i):
    ...
    if isinstance(part["custom_response"]["responses"], list):
        ...
        layout, n = re.subn("(?<=<)([0-9]+)(?=>)", lambda m: str(int(m.group(1))+identifier-1), layout)
...
master = env.get_template("master_exam.xml")
